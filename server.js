...
const sendAttendanceEmail = async () => {
    try {
        ...
        await page.goto('https://mujslcm.jaipur.manipal.edu/Student/Academic/AttendanceSummaryForStudent/', {
            waitUntil: 'networkidle2',
        });

        console.log('Waiting for Attendance Table...');
        await page.waitForSelector('#dvDetail > div', { visible: true, timeout: 60000 });
        ...
        const attendanceTableHtml = `
            <table border="1" style="border-collapse: collapse; width: 100%;">
                ...
                <tbody>
                    ${attendanceData.map(row => `<tr>${row.map(cell => `<td>${cell}</td>`).join('')}</tr>`).join('')}
                </tbody>
            </table>
        `;
